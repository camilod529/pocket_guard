...
- [ ] Bug: editing a transfer's From/To accounts is impossible with only 2 accounts. Each dropdown excludes whatever the *other* field currently holds (`excludeAccountId`), so swapping From/To has nowhere to go without a 3rd account to use as a buffer. Needs a UX fix — likely replace the live mutual-exclusion filtering with a "from ≠ to" validation error (or a dedicated swap button) instead of removing the other option from the list entirely.
