...
- [x] QOL: Pull-to-refresh on the Calendar/transaction view and the Accounts view - the first use of `RefreshIndicator` in the app. Both screens' providers (`TransactionsNotifier`, `CategoriesNotifier`, `AccountsNotifier`) already exposed a `refresh()` method, so the actual work was making every `AsyncValue.when()` branch - not just the data/list one - genuinely scrollable, since `RefreshIndicator` only recognizes a pull gesture if its child subtree contains a real `Scrollable`; a bare loading/empty/error `Center(...)` would otherwise make pull-to-refresh silently do nothing whenever that branch was showing. Added a shared `RefreshablePlaceholder` widget (`CustomScrollView` + `SliverFillRemaining` + `AlwaysScrollableScrollPhysics`) for those branches, and `AlwaysScrollableScrollPhysics` on the real list views too (needed for short lists that don't overflow the viewport). Calendar view refreshes transactions/categories/accounts together in parallel, since all three feed what's rendered there.
...
| Feature              | Description                                                                        |
| -------------------- | ---------------------------------------------------------------------------------- |
| **Multi-Currency**   | Centralize currency management: a fixed selectable currency list (replacing today's free-typed 3-letter account currency), a daily-refreshed exchange-rate source, and converting transactions to a budget/goal/report's currency instead of the current approach of just filtering out non-matching currencies. |
...
- **Budgets are currency-scoped, not currency-converting.** `Budget.currency` picks which of the user's account currencies a budget tracks (defaulted automatically when only one exists across accounts), and `budgetProgressProvider` only sums transactions from accounts in that same currency — transactions from other-currency accounts in the same category are silently excluded from the total rather than converted. Correct for single-currency users (the common case); for multi-currency users a budgeted category's "spent" figure will undercount. Real fix is the **Multi-Currency** item above (exchange rates + conversion) — deliberately deferred since it's a much larger, separate piece of work, not something to fold into the budget engine.
- **Removed dead per-exception-type error messaging infrastructure (`UserMessageKey`/`DataException.userMessage`) — never wired to the UI.** A l10n audit found 10 keys (`error_db_closed`, `error_storage_full`, `error_foreign_key_violation`, etc.) only ever referenced by a `userMessage` field on `DataException` subclasses that nothing in the UI read; screens all show one generic error message regardless of exception type. It was also further along than it looked but still broken: `DriftExceptionHandler._extractConstraintMessage` returned `'error_not_null_constraint'`/`'error_check_constraint'`, keys that were never even added to the arb files. Removed the whole mechanism rather than finish it, to keep this a cleanup and not new feature work. A real implementation would need something (a snackbar/dialog helper) to actually read `exception.userMessage` per catch site instead of the current generic handling, plus reinstating localized keys for each case — the `entityName`/`operation`/`fieldName`/`referencedTable` params were left in place on the exception constructors as the natural hooks for that.

---

## Suggested Tech Stack for Growth

- **State Management:** Since you have complex CRUD across accounts/balances, ensure you are using **Riverpod** or **Bloc** to keep the logic clean. _(Already on Riverpod.)_
- **Local Database:** If you aren't already, **Isar** or **Drift** are much faster than standard Sqflite for financial apps with many relations. _(Already on Drift.)_
- **Charts:** Use the `fl_chart` package — it's the standard for Flutter and highly customizable. _(Already in use.)_
